...
void purchaseFileRead(){
	...
	while(fscanf(fp, "%u\t%lf\t%lf\t%lf\t%s\t%s\t%lu", &Purchase[i].id, &Purchase[i].totalPrice, &Purchase[i].totalDiscount, &Purchase[i].totalProfit, Purchase[i].customerId, Purchase[i].personnelId, &Purchase[i].timestamp) != EOF){
		i++;
	}
